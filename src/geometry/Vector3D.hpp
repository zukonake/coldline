...
struct Vector3D
{
	...
	constexpr Vector3D( Vector2D< T > const &xy, T const &z ) noexcept;
	template< typename U >
...
template< typename T >
constexpr Vector3D< T >::Vector3D( Vector2D< T > const &xy, T const &z ) noexcept :
	x( xy.x ),
	y( xy.y ),
	z( z )
{
...
template< typename T >
template< typename U >
constexpr Vector3D< T >::operator U()
{
	return U( x, y, z );
}
